...
class Logger(object):
    def __init__(self, log_dir: str = None):
        self.logger = logging.getLogger('ShippingPath')

        # 输出到控制台
        console_handler = logging.StreamHandler()
        # 输出到文件
        log_dir = log_dir if log_dir else './logger/logs'
        ...
        try:
            ...
            # Fallback if no write permission (e.g. root owned log file)
            file_handler = None
    ...
    def clear_all_history_logs(log_dir: str = None):
        ...
        if not os.path.exists(log_dir):
            return
        LOG.warning("Clearing all history logs ...")
        for root, _, files in os.walk(log_dir):
            for name in files:
                LOG.info(f"Log {os.path.join(root, name)} is cleared ...")
